...
export default function SlotsPage() {
  ...
  const { data: slots, isFetching, refetch } = useQuery<SlotDto[]>({
    ...
    enabled: !!venueId,
    queryFn: () =>
      apiRequestJson("GET", `/api/venues/${venueId}/slots?from=${from}&to=${to}&serviceDuration=${duration}`),
  });

  useEffect(() => {
    if (!venueId && courts && courts.length) {
      setVenueId(courts[0].id);
    }
    if (!customerId && customers && customers.length) {
      setCustomerId(customers[0].id);
    }
  }, [courts, venueId, customers, customerId]);
  ...
  const holdAndBook = useMutation({
    ...
    onError: (err: any) => {
      toast({ title: "Could not book", description: err?.message || "Conflict", variant: "destructive" });
    },
  });

  const list = useMemo(() => slots || [], [slots]);
  const court = courts?.find((c) => c.id === venueId);
    ...
    <div className="p-6 space-y-4">
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <div>
            ...
            <p className="text-muted-foreground">Find and book available slots per venue.</p>
          </div>
          <div className="flex gap-2 flex-wrap items-center">
          <Select value={venueId} onValueChange={setVenueId}>
            <SelectTrigger className="w-56" data-testid="select-venue">
          ...
          <Input type="number" min={15} step={15} className="w-28" value={duration} onChange={(e) => setDuration(Number(e.target.value || 60))} />
          <Select value={customerId} onValueChange={setCustomerId}>
            <SelectTrigger className="w-52" data-testid="select-customer">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>{court ? `${court.name}` : "Select a venue"}</CardDescription>
        </CardHeader>
          ...
          {list.map((slot) => (
            <div key={`${slot.startAtUtc}-${slot.endAtUtc}`} className="border rounded-md p-3 space-y-2">
              ...
              <div className="flex items-center gap-2 text-xs text-muted-foreground">
                <Users className="h-3 w-3" /> {customers?.find((c) => c.id === customerId)?.name || "Select customer"}
              </div>
              <div className="flex justify-between items-center">
                ...
                <Button size="sm" onClick={() => holdAndBook.mutate(slot)} disabled={holdAndBook.isPending || !customerId}>
                  {holdAndBook.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <><ShieldCheck className="h-4 w-4 mr-1" />Hold & Book</>}
                </Button>
              </div>
            </div>
          ))}
...
function holdIdFrom(obj: any) {
  return obj?.data?.id || obj?.data?._id || obj?.id || obj?._id;
}
